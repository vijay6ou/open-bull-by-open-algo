...
Converts OpenBull symbols to Fyers HSM format for WebSocket streaming.
Uses the openbull symtoken cache for brsymbol mapping.

Ported from openalgo's broker/fyers/streaming/fyers_token_converter.py with
two adjustments:
  - ``get_br_symbol`` resolves from openbull's in-memory symtoken cache
    (``backend.broker.upstox.mapping.order_data.get_brsymbol_from_cache``)
    instead of openalgo's ``database.token_db.get_br_symbol``.
  - ``/data/symbol-token`` calls go through openbull's shared httpx client
    so requests bring our connection pooling, retries, and proxy config —
    no top-level ``requests`` dependency needed.
"""
...
from backend.broker.upstox.mapping.order_data import (
    get_brsymbol_from_cache as get_br_symbol,
)
