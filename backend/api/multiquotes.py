"""
External API - Multi-quotes endpoint.
...
async def api_multiquotes(request: Request):
    """Get quotes for multiple symbols via the external API."""
    ...
    try:
        ...
        logger.exception("Unexpected error in multiquotes endpoint")
    ...
    if not symbols or not isinstance(symbols, list):
        return JSONResponse(
            content={"status": "error", "message": "symbols list is required (e.g. [{\"symbol\":\"NHPC\",\"exchange\":\"NSE\"}])"},
            status_code=400,
        )
