/**
 * Template gallery — card grid with mini SVG payoff icons, direction tabs,
 * and a search box. Replaces the old single-line <select> picker.
 *
 * Each card draws the template's `payoffPath` inside a 100×40 SVG so the
 * shape of the payoff is recognisable at a glance (curve goes up = profit
 * up, etc.). Clicking the card calls `onPick(template)` — the parent page
 * is responsible for resolving the template's strikeOffsets to absolute
 * strikes via the chain context, prefilling entry_price from LTP, and
 * pushing the resulting legs onto the builder. None of that lives here.
 *
 * Math note: this component is presentation-only. It does NOT compute
 * P&L / Greeks / margin — those come from the server snapshot endpoint
 * once the legs land in state.
 */
...
interface Props {
  onPick: (template: StrategyTemplate) => void;
  /** Disable while no expiry / chain is loaded — the parent decides. */
  disabled?: boolean;
}
...
/** SVG payoff icon — drawn from the template's `payoffPath` d-string. */
...
export function TemplateGrid({ onPick, disabled = false }: Props) {
  const [direction, setDirection] = useState<Direction>("BULLISH");
  const [query, setQuery] = useState("");

  const counts = useMemo(() => directionCounts(), []);
  const grouped = useMemo(() => templatesByDirection(), []);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    const list = grouped[direction];
    if (!q) return list;
    return list.filter(
      (t) => t.name.toLowerCase().includes(q) || t.description.toLowerCase().includes(q),
    );
  }, [direction, query, grouped]);

  const total = STRATEGY_TEMPLATES.length;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-sm font-semibold leading-none">Strategy Templates</h3>
          <p className="mt-1 text-[11px] text-muted-foreground">
            {total} presets · click a card to load its legs
          </p>
        </div>
        ...
        <div className="relative sm:max-w-[220px] sm:flex-1">
          ...
          <input
            ...
            className="h-8 w-full rounded-md border border-input bg-background pl-8 pr-2 text-xs outline-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/50"
          />
        </div>
      </div>
